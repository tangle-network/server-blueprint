...
BINARY_PATH="./target/debug/mcp-blueprint-cli"
...
if [ "$run_blueprint" = "y" ]; then
    ...
    RUST_LOG=blueprint-rejection=trace,tangle-producer=debug,tangle-consumer=trace,blueprint-router=trace,blueprint-runner=trace,mcp_blueprint=debug,mcp_blueprint_cli=debug \
